...
let count_ones arr =
  Array.iteri
    (fun i n ->
      if n != 0 then Printf.printf "%d: %d %b %b\n" i n (n != 0) (n == 0)
      else ())
    arr
...
let calc_energy time size result =
  ...
  *)
  ...
  let energy3 =
    match result with Some _ -> energy2 | None -> energy2 *. 0.33
  in

  (* Make sure that we don't go over limit. *)
  let energy_pre_cap = energy3 in
  let energy_capped_top =
    if energy_pre_cap > havoc_max_mult *. 100.0 then havoc_max_mult *. 100.0
    else energy_pre_cap
  in
  let energy_capped_bot =
    if energy_capped_top < havoc_min then havoc_min else energy_capped_top
  in
