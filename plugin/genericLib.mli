...
val interp_open_coq_expr :
  Environ.env -> Evd.evar_map -> coq_expr -> EConstr.constr
...
type matcher_pat =
  ...
  | MatchU of var
  | MatchParameter of
      ty_param (* Should become hole in pattern, so no binding *)

val matcher_pat_to_string : matcher_pat -> string

val construct_match :
  coq_expr ->
  ?catch_all:coq_expr option ->
  (matcher_pat * coq_expr) list ->
  coq_expr

val construct_match_with_return :
  coq_expr ->
  ?catch_all:coq_expr option ->
  string ->
  (var -> coq_expr) ->
...
val dep_fold_ty :
  ('a -> dep_type -> 'a) ->
  ('a -> var -> dep_type -> 'a) ->
  (ty_ctr * dep_type list -> 'a) ->
  (constructor * dep_type list -> 'a) ->
  (ty_param -> 'a) ->
  (var -> 'a) ->
  dep_type ->
  'a
