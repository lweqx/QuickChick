...
let rec_method (generator_body : coq_expr) (hleq : var) (ih : var)
    (s2 : coq_expr) (n : int) (l : coq_expr list) : atyp =
...
let bind (opt : bool) (m : atyp) (x : string) (f : var -> btyp) : btyp =
  ...
  ( (* gen *)
    ...
    (* mon *)
    (if opt then semBindOptSizeOpt_subset_compat
     else semBindSizeOpt_subset_compat)
      mon
      (gFun [ x ] (fun [ x ] -> monf x)) )
...
let ret_type_dec (s : var) (v : var) (left1 : coq_expr) (right1 : coq_expr)
...
let check_expr (s1 : coq_expr) (s2 : coq_expr) (n : int) (scrut : coq_expr)
    (left : btyp) (right : btyp) =
...
let match_inp (s1 : coq_expr) (s2 : coq_expr) (inp : var) (pat : matcher_pat)
    (left : btyp) (right : btyp) =
  let lgen, lmon = left in
  let rgen, rmon = right in
  let proof_typ s v =
  ...
  ( (* gen *)
    (fun s ->
      construct_match_with_return (gVar inp)
        ~catch_all:(Some (rgen s))
        "v"
        (fun v -> hole)
        [ (pat, lgen s) ]),
    (* mon *)
    gFun [ "s" ] (fun [ s ] ->
        construct_match_with_return (gVar inp)
          ~catch_all:(Some (gApp rmon [ gVar s ]))
          "v" (proof_typ s)
...
let bigcupf s =
  gFun [ "x" ] (fun [ x ] ->
      set_int (isSomeSet hole) (semGenSize (gSnd (gVar x)) s))

let genSizedSTSMon_body (class_name : string) (gen_ctr : ty_ctr)
  ...
  let ind_gens (input_names : var list) (size : coq_expr) (rec_name : coq_expr)
      =
    ind_gens size full_gtyp gen_ctr dep_type ctrs input_names n
      register_arbitrary rec_name
  in
  ...
  let handle_branch' s1 s2 hleq ih (ins : var list) =
    handle_branch n dep_type ins (fail_exp full_gtyp) (ret_exp full_gtyp)
      class_method class_methodST (rec_method generator_body hleq ih s2)
      bind stMaybe (check_expr s1 s2) (match_inp s1 s2) (failwith "zoe fix me!")
      gen_ctr (fun _ -> ())
  in
    ...
    let cases =
      List.fold_right
        (fun (c : dep_ctr) (exp : coq_expr) ->
          ...
          in
          if b then bigcup_cons_setI_subset_compat_backtrack_weak p exp
          else bigcup_cons_setI_subset_pres_backtrack_weak exp)
        ctrs
        (gFun [ "s" ] (fun [ s ] -> bigcup_nil_setI hole hole hole))
    in
    gFun [ "s" ] (fun [ s ] ->
        subset_respects_set_eq
          ...
          (setI_set_eq_r
             (semBacktrackSize
                (gList (add_freq (ind_gens inputs (gVar s2) generator_body)))
                (gVar s)))
          (isSome_subset
             (setI_subset_compat set_incl_refl (gApp cases [ gVar s ]))))
  in
  ...
  let ind_case s1 s2 hleq ih (inputs : var list) =
    let cases =
      List.fold_right
        (fun (c : dep_ctr) (exp : coq_expr) ->
          let (_, p), b = handle_branch' (gVar s1) (gVar s2) hleq ih inputs c in
          bigcup_cons_setI_subset_compat_backtrack_weak p exp)
        ctrs
        (gFun [ "s" ] (fun [ s ] ->
             bigcup_nil_setI (bigcupf (gVar s)) hole hole))
    in
    gFun [ "s" ] (fun [ s ] ->
        subset_respects_set_eq
          (setI_set_eq_r
             (semBacktrackSize
                (gList (add_freq (ind_gens inputs (gVar s1) generator_body)))
                (gVar s)))
          (setI_set_eq_r
             (semBacktrackSize
                (gList (add_freq (ind_gens inputs (gVar s2) generator_body)))
  ...
  let ret_type s1 s2 =
    gProdWithArgs inputs (fun inps ->
        let inps = List.map gVar inps in
        gImpl (gLeq s1 s2)
          (gProdWithArgs
             [ gArg ~assumName:(gVar (fresh_name "s")) () ]
             (fun [ s ] ->
               let s = gVar s in
               set_incl
                 (set_int (isSomeSet hole)
                    (semGenSize (gApp generator_body (s1 :: inps)) s))
                 (set_int (isSomeSet hole)
                    (semGenSize (gApp generator_body (s2 :: inps)) s)))))
  in

  let out_type =
    gFun [ "s1" ] (fun [ s1 ] ->
        gProdWithArgs
          [ gArg ~assumName:(gVar (fresh_name "s2")) () ]
          (fun [ s2 ] -> ret_type (gVar s1) (gVar s2)))
  in

  let in_type s1 = gFun [ "s2" ] (fun [ s2 ] -> ret_type s1 (gVar s2)) in

  let mon_proof =
    gFun [ "s"; "s1"; "s2"; "Hleq" ] (fun [ s; s1; s2; hleq ] ->
        gApp
          (nat_ind (* outer induction *)
             ...
             (nat_ind
                ...
                (in_type (gInt 0))
                (* reflexivity *)
                (gFunWithArgs inputs (fun inps ->
                     gFun [ "Hleq"; "s" ] (fun [ hleq; s ] -> set_incl_refl)))
                (gFun [ "s2"; "IHs2" ] (fun [ s2; _ ] ->
                     gFunWithArgs inputs (fun inps ->
                         gFun [ "Hleq" ] (fun [ hleq ] ->
                             base_case s2 hleq inps)))))
             (* inductive case -- inner induction *)
             (gFun [ "s1"; "IHs1" ] (fun [ s1; ihs1 ] ->
                  nat_ind
                    (* inner type *)
                    ...
                    (gFunWithArgs inputs (fun inps ->
                         gFun [ "Hleq" ] (fun [ hleq ] ->
                             false_ind hole (lt0_False (gVar hleq)))))
                    (* inductive case *)
                    (gFun [ "s2"; "IHs2" ] (fun [ s2; _ ] ->
                         gFunWithArgs inputs (fun inps ->
                             gFun [ "Hleq" ] (fun [ hleq ] ->
