...
let base_ctrs ty_ctr ctrs =
  List.filter (fun (_, ty) -> isBaseBranch ty_ctr ty) ctrs
...
let sized_decl ty_ctr ctrs =
  let sizeOf_body x =
    ...
    gRecFunIn "aux_size" [ "x'" ]
      (fun (aux_size, [ x' ]) ->
        gMatch (gVar x') (List.map (create_branch aux_size) ctrs))
...
let rec dropIH cty ty_ctr l =
  match cty with
  | Arrow (ty1, ty2) -> (
      if sameTypeCtr ty_ctr ty1 then
        match l with
        | x :: _ihx :: l ->
...
let zeroEqProof ty_ctr ctrs (ind_scheme : coq_expr) size zeroType zeroSized
    iargs =
               ...
               fun [ _w'; hw' ] ->
                 gMatch (gVar hw')
                   [
                     ( injectCtr "conj",
                       [ hc1; hc2 ],
                       fun [ _hc1; hc2 ] -> elim_set hc2 ty2 (n + 1) );
                   ] );
           ] [@ocaml.warning "-8"])
  ...
  let rec elim_unions h ctrs =
    ...
    | (_ctr, ty) :: ctrs' ->
        gMatch (gVar h)
          [
            (injectCtr "or_introl", [ "H1" ], fun [ h1 ] -> elim_set h1 ty 0);
            (injectCtr "or_intror", [ "H1" ], fun [ h1 ] -> elim_unions h1 ctrs');
          ]
  in
  ...
  let rec intro_set ty ctr_args ctr acc =
    ...
    | Arrow (_ty1, ty2) -> (
        ...
        | arg :: ctr_args' ->
            gExIntro_impl arg
              (gConjIntro gI (intro_set ty2 ctr_args' ctr (arg :: acc)))
  ...
  let rec intro_unions ctrs args curr_ctr =
    match ctrs with
    | [ (ctr, ty) ] ->
        if ctr = curr_ctr then intro_set ty args ctr []
        else failwith "Internal: cannot find constructor"
    | (ctr, ty) :: ctrs' ->
        if ctr = curr_ctr then gOrIntroL (intro_set ty args ctr [])
        else gOrIntroR (intro_unions ctrs' args curr_ctr)
  in
  ...
  let create_case (ctr, ty) =
    ...
    gFun iargs (fun iargs ->
        ...
        let elem =
          gApp ~explicit:true (gCtr ctr) (coqTyParams @ List.map gVar args)
        in
        ...
        if isBaseBranch ty_ctr ty then
          gConjIntro
            ...
            (gFunTyped
               [ ("H1", rhs) ]
               (fun [ _h1 ] -> intro_unions base_ctrs (List.map gVar args) ctr))
        else
          gConjIntro
...
let succEqProof ty_ctr ctrs (ind_scheme : coq_expr) succType succSized iargs =
                ...
                gMatch (gVar hw')
                  [
                    ( injectCtr "conj",
                      ...
                      fun [ hc1; hc2 ] ->
                        let leq' =
                          if sameTypeCtr ty_ctr ty1 then gVar hc1 :: leq
                          else leq
                        in
    ...
    | _ ->
        if ctr_flag then
          let rec leq_proof = function
            | [ h ] -> h
            | h :: hs ->
                gApp (gInject "max_lub_ssr")
                  [ hole; hole; gSucc (gVar size); h; leq_proof hs ]
          in
          gMatch (gVar h)
            [ (injectCtr "erefl", [], fun [] -> leq_proof (List.rev leq)) ]
        ...
        gMatch (gVar h)
          [
            ( injectCtr "or_introl",
              [ "H1" ],
              fun [ h1 ] ->
                elim_set h1 [] ty 0 (fun x -> x) (curr_ctr = ctr) size );
            ( injectCtr "or_intror",
              [ "H1" ],
  ...
  let rec intro_set leq ty ctr_args iargs ctr acc =
    ...
    | Arrow (ty1, ty2) -> (
        ...
        | arg :: ctr_args' ->
            ...
            gExIntro_impl arg
              (gConjIntro leq_l
                 (intro_set leq_r ty2 ctr_args' iargs' ctr (arg :: acc)))
        | [] -> failwith "Internal: wrong number of arguments")
  ...
  let rec intro_unions h ctrs args ihargs curr_ctr =
    match ctrs with
    | [ (ctr, ty) ] ->
        if ctr = curr_ctr then intro_set (gVar h) ty args ihargs ctr []
        else failwith "Internal: cannot find constructor"
    | (ctr, ty) :: ctrs' ->
        if ctr = curr_ctr then
          gOrIntroL (intro_set (gVar h) ty args ihargs ctr [])
        else gOrIntroR (intro_unions h ctrs' args ihargs curr_ctr)
  in
  ...
  let create_case size (ctr, ty) =
    ...
    gFun iargs (fun iargs ->
        ...
        in
        let leq_size size =
          set_suchThat "x" full_dt (fun x -> gle (gApp sizeM [ gVar x ]) size)
        in
        ...
        if isBaseBranch ty_ctr ty then
          gConjIntro
            (gFunTyped
               [ ("H1", lhs) ]
               (fun [ _h1 ] -> gApp (gInject "leq0n") [ hole ]))
            (gFunTyped
               [ ("H1", rhs) ]
               (fun [ h1 ] ->
                 intro_unions h1 ctrs (List.map gVar args) ihargs ctr))
        else
          gConjIntro
            (gFunTyped
               [ ("H1", lhs) ]
               (fun [ h1 ] -> elim_unions h1 ctrs ctr size))
            (gFunTyped
               [ ("H1", rhs) ]
               (fun [ h1 ] ->
                 intro_unions h1 ctrs (List.map gVar args) ihargs ctr)))
  ...
  gFun [ "size" ] (fun [ size ] ->
      gApp ~explicit:true ind_scheme
        (coqTyParams @ (succType size :: proofs size)))
...
let sizeEqType ty_ctr ctrs ind_scheme iargs =
  ...
  let create_branch set tps (ctr, ty) =
    let rec aux i acc ty : coq_expr =
      ...
      | Arrow (ty1, ty2) ->
          ...
          set_bigcup fi
            (if sameTypeCtr ty_ctr ty1 then set
             else gFun [ fi ] (fun _ -> gInject "True"))
            (fun f -> aux (i + 1) (f :: acc) ty2)
      | _ ->
          set_singleton
            (gApp ~explicit:true (gCtr ctr)
               (tps @ List.map gVar (List.rev acc)))
    in
    aux 0 [] ty
  in

  let lhs set ctrs =
    set_unions (List.map (create_branch set coqTyParams) ctrs)
  in
  let rhs size =
    set_suchThat "x" full_dt (fun x -> gEq (gApp sizeM [ gVar x ]) size)
  in

  let zeroSized = lhs hole bases in
  let succSized =
    gFunWithArgs
      [ gArg ~assumName:(gInject "set") () ]
      (fun [ set ] -> lhs (gVar set) ctrs)
  in

  let zeroType =
    gFun [ "f" ] (fun [ f ] ->
        gIff (gApp zeroSized [ gVar f ]) (gApp (rhs (gInt 0)) [ gVar f ]))
  in
  ...
  let succType size =
    gFun [ "f" ] (fun [ f ] ->
        gIff
          (gApp (gApp succSized [ set_leq (gVar size) ]) [ gVar f ])
          (gApp (set_leq (gSucc (gVar size))) [ gVar f ]))
  in

  let zeroSized_spec =
    zeroEqProof ty_ctr ctrs ind_scheme sizeM zeroType zeroSized iargs
  in
  let succSized_spec =
    succEqProof ty_ctr ctrs ind_scheme succType succSized iargs
  in
  ...
  gRecord
    [
      ("zeroSized", zeroSized);
      ("succSized", succSized);
      ("zeroSized_spec", zeroSized_spec);
      ("succSized_spec", succSized_spec);
    ]
