...
module UM :
  CMap.ExtS with type key = Unknown.t and module Set := Set.Make(UnknownOrd)
...
val raiseMatch :
  ...
  range list ->
  EqSet.t ->
  (umap * GenericLib.matcher_pat * EqSet.t) option
...
val dt_to_coq_expr : range UM.t -> GenericLib.dep_type -> GenericLib.coq_expr
...
module CMap :
  CMap.ExtS
    with type key = GenericLib.OrdDepType.t
     and module Set := Set.Make(GenericLib.OrdDepType)
...
type mode =
  | Recursive of
      (Unknown.t * GenericLib.dep_type) list
      * (Unknown.t * GenericLib.dep_type) list
      * range list
