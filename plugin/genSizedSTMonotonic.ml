...
let rec_method (ih : var) (n : int) (l : coq_expr list) = gApp (gVar ih) l
...
let match_inp (inp : var) (pat : matcher_pat) (left : coq_expr)
  ...
  in
  construct_match_with_return (gVar inp) ~catch_all:(Some right) "s"
    (fun v -> ret_type v ret)
    [ (pat, left) ]

let genSizedSTMon_body (class_name : string) (gen_ctr : ty_ctr)
  ...
  let aux_arb (input_names : var list) (rec_name : coq_expr) size vars =
    gMatch (gVar size)
      [
        ( injectCtr "O",
          [],
          fun _ -> uniform_backtracking (base_gens input_names rec_name) );
        ( injectCtr "S",
          [ "size'" ],
          fun [ size' ] ->
  ...
  let generator_body (input_names : var list) : coq_expr =
    gRecFunInWithArgs ~assumType:gen_type "aux_arb"
      ...
      (fun (rec_name, size :: vars) ->
        aux_arb input_names (gVar rec_name) size vars)
      (fun x -> gVar x)
  in
  ...
  let base_case =
    let handle_branch' (inputs : var list) =
      handle_branch n dep_type inputs fail_exp ret_exp class_method
        class_methodST
        (rec_method (make_up_name ()))
        bind stMaybe check_expr match_inp gLetIn gen_ctr
        (fun _ -> ())
    in
        ...
        backtrackSizeMonotonicOpt
          ...
          (List.fold_right
             (fun (c : dep_ctr) (exp : coq_expr) ->
               ...
               if b then cons_subset hole hole hole p exp else exp)
  ...
  let ind_case =
    let handle_branch' (ih : var) (size : var) (inputs : var list) =
      handle_branch n dep_type inputs fail_exp ret_exp class_method
        class_methodST (rec_method ih) bind stMaybe check_expr match_inp
        (failwith "zoe fix me!") gen_ctr (fun _ -> ())
    in
    gFun [ "size"; "IHs" ] (fun [ size; ihs ] ->
        gFunWithArgs inputs (fun inputs ->
            backtrackSizeMonotonicOpt
    ...
    gFun [ "s" ] (fun [ s ] ->
        gProdWithArgs inputs (fun inputs ->
            gApp (gInject class_name)
              [
                gApp ~explicit:true
                  (gInject "arbitrarySizeST")
                  [ full_gtyp; full_pred (List.map gVar inputs); hole; gVar s ];
              ]))
  in

  let mon_proof = gApp (gInject "nat_ind") [ ret_type; base_case; ind_case ] in
