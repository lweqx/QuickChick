...
let arbitrarySized_body (ty_ctr : ty_ctr) (ctrs : ctr_rep list) iargs =
  ...
  in
  let isBaseBranch ty =
    fold_ty' (fun b ty' -> b && not (isCurrentTyCtr ty')) true ty
  in
  ...
  let create_for_branch tyParams rec_name size (ctr, ty) =
    let rec aux i acc ty : coq_expr =
      ...
      | Arrow (ty1, ty2) ->
          bindGen
            (if isCurrentTyCtr ty1 then gApp (gVar rec_name) [ gVar size ]
             else gInject "arbitrary")
            (Printf.sprintf "p%d" i)
            (fun pi -> aux (i + 1) (gVar pi :: acc) ty2)
      | _ ->
          returnGen (gApp ~explicit:true (gCtr ctr) (tyParams @ List.rev acc))
    in
    aux 0 [] ty
  in
  ...
  gRecFunInWithArgs "arb_aux"
    [ gArg ~assumName:(gInject "size") () ]
    (fun (aux_arb, [ size ]) ->
      gMatch (gVar size)
        [
          ( injectCtr "O",
            [],
            fun _ ->
              oneof (List.map (create_for_branch tyParams aux_arb size) bases)
          );
          ( injectCtr "S",
            [ "size'" ],
            fun [ size' ] ->
                ...
                (List.map
                   (fun (ctr, ty') ->
                     ( Weightmap.lookup_weight ctr size',
                       create_for_branch tyParams aux_arb size' (ctr, ty') ))
                   ctrs) );
        ])
    (fun x -> gVar x)
            ...
            (fun allParams v ty' ->
              ...
              lst_appends
                (if isCurrentTyCtr ty' then
                   [
                     gList [ gVar v ];
                     gApp (gInject "List.map")
                       [ liftNth; gApp (gVar aux_shrink) [ gVar v ] ];
                   ]
                 else
                   [
                     gApp (gInject "List.map")
                       [ liftNth; gApp (gInject "shrink") [ gVar v ] ];
                   ]))
            lst_append list_nil ty )
      in
              ...
              str_append
                ...
                (fold_ty_vars
                   (fun _ v ty' ->
                     smart_paren
                       (gApp
                          (if isCurrentTyCtr ty' then gVar aux
                           else gInject "show")
                          [ gVar v ]))
                   (fun s1 s2 ->
