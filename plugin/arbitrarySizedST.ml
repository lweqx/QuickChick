...
(* Advanced Generators *)
let arbitrarySizedST (gen_ctr : ty_ctr) (ty_params : ty_param list)
    ...
    gMatch (gVar size)
      [
        ...
        ( injectCtr "S",
          ...
          fun [ size' ] ->
            let weights =
              List.map (fun (c, _) -> Weightmap.lookup_weight c size') ctrs
            in
            backtracking
              (List.combine weights
                 (ind_gens init_size (gVar size') full_gtyp gen_ctr dep_type
                    ctrs rec_name input_ranges init_umap init_tmap result)) );
      ]
  in
