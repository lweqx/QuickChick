...
let fail_exp (dt : coq_expr) : btyp =
  ( (* set *)
    ...
    (* comp *)
    gFun [ "x"; "Hx" ] (fun [ x; hx ] ->
        false_ind hole (imset_set0_incl hole hole (gVar hx))),
    (* sound *)
    gFun [ "x"; "Hx" ] (fun [ x; hx ] ->
        gOrIntroR (rewrite_set_l (semReturn hole) (gVar hx))) )
...
let ret_exp (dt : coq_expr) (c : coq_expr) : btyp =
  ( (* set *)
    ...
    gFun [ "x"; "Hx" ] (fun [ x; hx ] ->
        rewrite hole
          (imset_singl_incl hole hole hole (gVar hx))
          (rewrite_set_r (semReturn hole) (gEqRefl hole))),
    (* sound *)
    gFun [ "x"; "Hx" ] (fun [ x; hx ] ->
        gOrIntroL
          (gExIntro_impl hole
             (gConjIntro (gEqRefl hole)
                (rewrite_set_l (semReturn hole) (gVar hx))))) )
...
let class_methodST (n : int) (pred : coq_expr) : atyp =
  let cproof =
    gApp ~explicit:true (gInject "STCorrect") [ hole; pred; hole; hole ]
  in
...
let rec_method (inputs : arg list) (setinst : coq_expr)
    (generator_body : coq_expr) (moninst : coq_expr) (ih : var)
    (size : coq_expr) (n : int) (l : coq_expr list) : atyp =
  let iter_body args : coq_expr =
    appinst "DependentClasses.iter" setinst size args
  in
  let gen_body args : coq_expr = gApp generator_body (size :: args) in
...
let bind (opt : bool) (m : atyp) (x : string) (f : var -> btyp) : btyp =
  ...
  let setf x =
    let set, _, _, _, _ = f x in
    set
  in
  let genf x =
    let _, gen, _, _, _ = f x in
    gen
  in
  let monf x =
    let _, _, mon, _, _ = f x in
    mon
  in
  let compf x =
    let _, _, _, pr, _ = f x in
    pr
  in
  let soundf x =
    let _, _, _, _, pr = f x in
    pr
  in
  let hxc = "Hc_" ^ x in
  let hx = "H_" ^ x in
  ...
  ( (* set *)
    ...
    (let bind =
       (if opt then semBindOptSizeMonotonicIncl_l
        else semBindSizeMonotonicIncl_l)
         gen
         (gFun [ x ] (fun [ x ] -> genf x))
         set
         (gFun [ x ] (fun [ x ] -> setf x))
...
let ret_comp matcher1 matcher2 =
  set_incl (imset (gInject "Some") matcher1) (semGen matcher2)

let ret_sound matcher1 matcher2 =
  set_incl (semGen matcher2)
    (set_union (imset (gInject "Some") matcher1) (set_singleton (gNone hole)))

let ret_type_dec (ret : coq_expr -> coq_expr -> coq_expr) (s : var)
    (left1 : coq_expr) (right1 : coq_expr) (left2 : coq_expr)
    (right2 : coq_expr) =
  ret
    (gMatch (gVar s)
       [
...
let ret_mon matcher = gApp (gInject "SizeMonotonicOpt") [ matcher ]
...
let check_expr (n : int) (scrut : coq_expr) (left : btyp) (right : btyp) =
  ...
  ( (* set *)
    gMatchReturn scrut "v" (* as clause *)
      (fun v -> hole)
      [
...
let match_inp (inp : var) (pat : matcher_pat) (left : btyp) (right : btyp) =
  ...
  in
  ( (* set *)
    construct_match_with_return (gVar inp) ~catch_all:(Some rset) "v"
      (fun v -> hole)
      [ (pat, lset) ],
    (* gen *)
    construct_match_with_return (gVar inp) ~catch_all:(Some rgen) "v"
      (fun v -> hole)
      [ (pat, lgen) ],
    (* mon *)
    construct_match_with_return (gVar inp) ~catch_all:(Some rmon) "v" mon_typ
      [ (pat, lmon) ],
    (* comp *)
    construct_match_with_return (gVar inp) ~catch_all:(Some rcomp) "v"
      (proof_typ ret_comp)
      [ (pat, lcomp) ],
    (* sound *)
    construct_match_with_return (gVar inp) ~catch_all:(Some rsound) "v"
      (proof_typ ret_sound)
      [ (pat, lsound) ] )

let stMaybe (opt : bool) (exp : atyp) (x : string)
    (checks : ((coq_expr -> coq_expr) * int) list) =
  ...
  let rec sumbools_to_bool x lst e fail =
    ...
    | (chk, _) :: lst' ->
        matchDec
          (chk (gVar x))
          (fun heq -> fail)
          (fun hneq -> sumbools_to_bool x lst' e fail)
  in
  let bool_pred checks =
    gFun [ x ] (fun [ x ] -> sumbools_to_bool x checks gTrueb gFalseb)
  in
  let hxs = "H_" ^ x in
  let ret_comp matcher1 matcher2 = gImpl matcher1 (gConj hole matcher2) in
  let ret_sound matcher1 matcher2 = gImpl (gConj hole matcher2) matcher1 in
  let rec sumbools_to_bool_comp (x : var) hx lst : coq_expr =
    match lst with
    | [] -> gConjIntro (gVar hx) (gEqRefl hole)
    | (chk, n) :: lst' ->
        let set d =
          gMatchReturn (gVar d) "s"
            (fun v -> gProp)
            [
              (injectCtr "left", [ "eq" ], fun _ -> gFalse);
              ( injectCtr "right",
                [ "neq" ],
                fun _ -> sumbools_to_bool x lst' (gApp set [ gVar x ]) gFalse );
            ]
        in
        let pred d =
          gIsTrue
          ...
          (gMatchReturn
             ...
             [
               ( injectCtr "left",
                 [ "heq" ],
                 fun [ heq ] ->
                   gFun [ hxs ] (fun [ hx ] -> false_ind hole (gVar hx)) );
               ( injectCtr "right",
                 [ "hneq" ],
                 fun [ hneq ] ->
                   gFun [ hxs ] (fun [ hx ] -> sumbools_to_bool_comp x hx lst')
               );
             ])
          [ gVar hx ]
  in
                       ...
                       gMatch (gVar hx)
                         [
                           ( injectCtr "conj",
                             [ "hl"; "hr" ],
                             fun [ hl; hr ] ->
                               false_ind hole (diff_false_true (gVar hr)) );
                         ]) );
               ( injectCtr "right",
                 [ "hneq" ],
                 fun [ hneq ] ->
                   gFun [ hxs ] (fun [ hx ] -> sumbools_to_bool_sound x hx lst')
               );
             ])
          [ gVar hx ]
  in
  ( (* set *)
    gFun [ x ] (fun [ x ] ->
        sumbools_to_bool x checks (gApp set [ gVar x ]) gFalse),
    (* gen *)
    gApp
      (gInject (if opt then "suchThatMaybeOpt" else "suchThatMaybe"))
      [
        gen (* Use the generator provided for base generator *);
        bool_pred checks;
      ],
    (* mon *)
    (if opt then suchThatMaybeOptMonotonicOpt else suchThatMaybeMonotonicOpt)
      mon (bool_pred checks),
    (* comp *)
    set_incl_trans
      (imset_incl
         (gFun [ x; hxs ] (fun [ x; hx ] -> sumbools_to_bool_comp x hx checks)))
    ...
    set_incl_trans
      ...
      (setU_set_subset_compat
         (imset_incl
            (gFun [ x; hxs ] (fun [ x; hx ] ->
                 sumbools_to_bool_sound x hx checks)))
         set_incl_refl) )

let genSizedSTCorr_body (class_name : string) (gen_ctr : ty_ctr)
    (ty_params : ty_param list) (ctrs : dep_ctr list) (dep_type : dep_type)
    (input_names : string list) (inputs : arg list) (n : int)
    (register_arbitrary : dep_type -> unit) (moninst : coq_expr)
    (geninst : coq_expr) (setinst : coq_expr) =
  (* type constructor *)
  ...
  let full_pred inputs =
    gFun [ "_forGen" ] (fun [ fg ] ->
        gApp full_dt (list_insert_nth (gVar fg) inputs (n - 1)))
  in

  let base_gens (input_names : var list) (rec_name : coq_expr) =
    base_gens (gInt 0) full_gtyp gen_ctr dep_type ctrs input_names n
      register_arbitrary rec_name
  in

  let ind_gens (input_names : var list) (size : var) (rec_name : coq_expr) =
    ind_gens (gVar size) full_gtyp gen_ctr dep_type ctrs input_names n
      register_arbitrary rec_name
  in
  ...
  let aux_arb (rec_name : coq_expr) size vars =
    gMatch (gVar size)
      [
        ( injectCtr "O",
          [],
          fun _ -> uniform_backtracking (base_gens vars rec_name) );
        ( injectCtr "S",
          [ "size'" ],
          fun [ size' ] -> uniform_backtracking (ind_gens vars size' rec_name)
        );
      ]
  in
  ...
  let generator_body : coq_expr =
    (* gInject "gen" *)
    gRecFunInWithArgs ~assumType:gen_type "aux_arb"
  ...
  let add_freq gens =
    List.map gPair (List.combine (List.map (fun _ -> gInt 1) gens) gens)
  in

  let handle_branch' (ih : var) (size : coq_expr) (ins : var list) =
    handle_branch n dep_type ins (fail_exp full_gtyp) (ret_exp full_gtyp)
      class_method class_methodST
      (rec_method inputs setinst generator_body moninst ih size)
      bind stMaybe check_expr match_inp (failwith "zoe fix me!") gen_ctr
      (fun _ -> ())
  in

  let some_proof hc =
    gMatch
      (in_imset hole hole hole hc)
      [
        ( injectCtr "ex_intro",
          [ "z"; "Heqz" ],
          fun [ z; heq ] ->
            rewrite_sym
              (gFun [ "x" ] (fun [ x ] -> isSome (gVar x)))
              (gVar heq) (isSomeSome hole) );
      ]
  in

  let base_case =
    gFunWithArgs inputs (fun inputs ->
        let (cases : coq_expr) =
          List.fold_right
            (fun (c : dep_ctr) (exp : coq_expr) ->
              ...
              in
              if b then
                imset_bigcup_setI_cons_subset_r (gProd hole hole) hole
                  (succ_neq_zero hole)
        ...
        in
        set_incl_trans cases
          (* (setU_subset_l hole cases) *)
          (semBacktrack_complete
             (gList (add_freq (base_gens inputs generator_body)))))
  in

  let ind_case =
    gFun [ "size"; "IHs" ] (fun [ s; ih ] ->
        gFunWithArgs inputs (fun inputs ->
            let cases =
              List.fold_right
                (fun (c : dep_ctr) (exp : coq_expr) ->
            ...
            set_incl_trans cases
              (semBacktrack_complete
                 (gList (add_freq (ind_gens inputs s generator_body))))))
  in

  let ret_type =
    gFun [ "size" ] (fun [ s ] ->
        gProdWithArgs inputs (fun inputs ->
            let inps = List.map gVar inputs in
            set_incl
              (* (imset (gInject "Some") (gApp (gInject "aux_iter") ((gVar s) :: inps))) *)
              (imset (gInject "Some")
                 (appinst "DependentClasses.iter" setinst (gVar s) inps))
  ...
  let base_case_sound =
    gFunWithArgs inputs (fun inputs ->
        let cases =
          List.fold_right
            (fun (c : dep_ctr) (exp : (coq_expr -> coq_expr) -> coq_expr) proof ->
              let (_, _, _, _, p), b =
                handle_branch' (make_up_name ()) (gInt 0) inputs c
              in
              if b then
                bigcup_cons_subset (gProd hole hole) hole
                  (set_incl_setI_r (proof p))
                  (exp (fun e -> lift_subset_pres_r (proof e)))
              else exp proof)
            ctrs
            (fun e -> bigcup_nil_subset)
            lift_subset_pres_l
        in
        set_incl_trans
          (semBacktrack_sound
             (gList (add_freq (base_gens inputs generator_body))))
          (set_incl_setU_l (bigcup_set_I_l cases)
        ...
        gProdWithArgs inputs (fun inputs ->
            ...
            set_incl
              ...
              (set_union
                 (imset (gInject "Some")
                    (appinst "DependentClasses.iter" setinst (gVar s) inps))
                 (set_singleton (gNone hole)))))
  in

  let sound_proof =
    gFun [ "size" ] (fun [ s ] ->
        gApp (gInject "nat_ind")
          ([ ret_type_sound; base_case_sound; ind_case_sound; gVar s ]
          @ List.map gVar input_vars))
  in

  let correct =
    gFun [ "s" ] (fun [ s ] ->
        isSome_set_eq (gApp sound_proof [ gVar s ]) (gApp com_proof [ gVar s ]))
  in
