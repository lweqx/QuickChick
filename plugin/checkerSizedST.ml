...
let instantiate_existential_methodST (n : int) (pred : coq_expr) =
  gApp ~explicit:true (gInject "enumSuchThat")
    [
      hole (* Implicit argument - type A *); pred; hole (* Implicit instance *);
    ]

let rec_method (rec_name : coq_expr) (init_size : coq_expr) (size : coq_expr)
    (n : int) (letbinds : unknown list option) (l : coq_expr list) =
  gApp rec_name (init_size :: size :: l)

(* For checkers, ignore the opt argument *)
let rec_bind (opt : bool) (m : coq_expr) (x : string) (f : var -> coq_expr) :
    coq_expr =
  gMatch m
    [
      ( injectCtr "Some",
...
let exist_bind (init_size : coq_expr) (opt : bool) (m : coq_expr) (x : string)
    (f : var -> coq_expr) : coq_expr =
  enumCheckerOpt m x f init_size
(*  (if opt then enumCheckerOpt else enumChecker) m x f init_size *)

let stMaybe (opt : bool) (g : coq_expr) (x : string)
  ...
  let rec sumbools_to_bool x lst =
    ...
    | (chk, _) :: lst' ->
        matchDec
          (chk (gVar x))
          (fun heq -> gFalseb)
          (fun hneq -> sumbools_to_bool x lst')
  in
  let bool_pred = gFun [ x ] (fun [ x ] -> sumbools_to_bool x checks) in
  gApp
    (gInject (if opt then "suchThatMaybeOpt" else "suchThatMaybe"))
    [ g (* Use the generator provided for base generator *); bool_pred ]
...
let check_expr (n : int) (scrut : coq_expr) (left : coq_expr) (right : coq_expr)
    (out_of_fuel : coq_expr) =
  gMatchReturn scrut "s" (* as clause *)
...
let match_inp (inp : var) (pat : matcher_pat) (left : coq_expr)
    (right : coq_expr) =
  ...
  let catch_case =
    ...
    | MatchCtr (c, ls) ->
        ...
        if
          num_of_ctrs c = 1
          && List.for_all
               (fun x -> match x with MatchU _ -> true | MatchCtr _ -> false)
               ls
        then None
        else Some right
    | _ -> failwith "Toplevel match not a constructor?"
  in
  construct_match_with_return (gVar inp) ~catch_all:catch_case "s"
    (fun v -> ret_type v ret)
    [ (pat, left) ]

type generator_kind = Base_gen | Ind_gen

(* hoisting out base and ind gen to be able to call them from proof generation *)
let construct_generators (kind : generator_kind) (init_size : coq_expr)
    (size : coq_expr) (full_gtyp : coq_expr) (gen_ctr : ty_ctr)
    (dep_type : dep_type) (ctrs : dep_ctr list) (rec_name : coq_expr)
    (input_ranges : range list) (init_umap : range UM.t)
    (init_tmap : dep_type UM.t) (result : Unknown.t) =
  msg_debug (str "Beginning checker construction" ++ fnl ());
  (* partially applied handle_branch *)
  let handle_branch' : dep_ctr -> coq_expr * bool =
    handle_branch
      [ "EnumSizedSuchThat"; "EnumSuchThat" ]
      dep_type init_size (fail_exp full_gtyp)
      (not_enough_fuel_exp full_gtyp)
      (ret_exp full_gtyp) instantiate_existential_method
      instantiate_existential_methodST (exist_bind init_size)
      (rec_method rec_name init_size size)
      rec_bind stMaybe check_expr match_inp gLetIn gLetTupleIn gen_ctr init_umap
      init_tmap input_ranges result
  in
  let all_gens = List.map handle_branch' ctrs in
  let padNone =
    if List.exists (fun gb -> not (snd gb)) all_gens then [ gNone gBool ]
    else []
  in
  match kind with
  ...
  | Ind_gen ->
      List.map fst
        (List.filter snd all_gens @ List.filter (fun x -> not (snd x)) all_gens)

let base_gens = construct_generators Base_gen
let ind_gens = construct_generators Ind_gen

(* Advanced Generators *)
let checkerSizedST (gen_ctr : ty_ctr) (ty_params : ty_param list)
    (ctrs : dep_ctr list) (dep_type : dep_type) (input_names : var list)
    (input_ranges : range list) (init_umap : range UM.t)
    (init_tmap : dep_type UM.t) (inputs : arg list) (result : Unknown.t)
    (rec_name : coq_expr) =
  (* type constructor *)
        ...
        ( injectCtr "O",
          ...
          fun _ ->
            checker_backtracking
              (base_gens init_size (gVar size) full_gtyp gen_ctr dep_type ctrs
                 rec_name input_ranges init_umap init_tmap result) );
        ( injectCtr "S",
          [ "size'" ],
          fun [ size' ] ->
            checker_backtracking
              (ind_gens init_size (gVar size') full_gtyp gen_ctr dep_type ctrs
                 rec_name input_ranges init_umap init_tmap result) );
      ]
  in
  ...
  let generator_body : coq_expr =
    ...
    gRecFunInWithArgs ~structRec:(Some sizeVar) ~assumType:gen_type "aux_arb"
      (gArg ~assumName:(gVar (fresh_name "init_size")) ()
      :: gArg ~assumName:(gVar sizeVar) ()
      :: inputs)
      (fun (rec_name, init_size :: size :: vars) ->
        aux_arb (gVar rec_name) (gVar init_size) size vars)
      (fun rec_name ->
        gFun [ "size" ] (fun [ size ] ->
            gApp (gVar rec_name)
              (gVar size :: gVar size
              :: List.map (fun i -> gVar (arg_to_var i)) inputs)))
  in
