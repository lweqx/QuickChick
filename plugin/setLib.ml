...
let imset f s = gApp (gInject "imset") [ f; s ]
let sub0set = gApp ~explicit:true (gInject "sub0set") [ hole; hole ]
...
let set_eq_refl x = gApp (gInject "set_eq_refl") [ x ]
let set_incl_refl = gApp ~explicit:true (gInject "subset_refl") [ hole; hole ]
let _incl_subset l1 l2 p = gApp (gInject "incl_subset") [ l1; l2; p ]
let incl_refl = gApp (gInject "incl_refl") [ hole ]

let incl_hd_same p =
  gApp ~explicit:true (gInject "incl_hd_same") [ hole; hole; hole; hole; p ]

let incl_tl p = gApp (gInject "incl_tl") [ hole; p ]
let setU_set_eq_compat x1 x2 = gApp (gInject "setU_set_eq_compat") [ x1; x2 ]
let setU_set0_r x1 x2 = gApp (gInject "setU_set0_r") [ x1; x2 ]
let set_eq_trans x1 x2 = gApp (gInject "set_eq_trans") [ x1; x2 ]
let set_incl_trans x1 x2 = gApp (gInject "subset_trans") [ x1; x2 ]
let setU_set0_l x1 x2 = gApp (gInject "setU_set0_l") [ x1; x2 ]
let setU_set0_neut_eq x1 x2 = gApp (gInject "setU_set0_neut_eq") [ x1; x2 ]
...
let subset_respects_set_eq p1 p2 p3 =
  gApp ~explicit:true
    (gInject "subset_respects_set_eq")
...
let incl_bigcupl p = gApp (gInject "incl_bigcupl") [ p ]
let incl_bigcup_compat p1 p2 = gApp (gInject "incl_bigcup_compat") [ p1; p2 ]
let imset_isSome s = gApp ~explicit:true (gInject "imset_isSome") [ hole; s ]
...
let setI_subset_compat p1 p2 =
  gApp ~explicit:true
    (gInject "setI_subset_compat")
    [ hole; hole; hole; hole; hole; p1; p2 ]

let nil_subset p = gApp (gInject "nil_subset") [ p ]

let cons_subset (hd : coq_expr) (tl : coq_expr) (p : coq_expr) (phd : coq_expr)
    (ptl : coq_expr) =
  gApp ~explicit:true (gInject "cons_subset") [ hole; hd; tl; p; phd; ptl ]

let setI_set_incl hsub1 hsub2 =
  gApp ~explicit:true (gInject "setI_set_incl")
    [ hole; hole; hole; hole; hsub1; hsub2 ]
...
let imset_union_incl s1 s2 f x hin =
  gApp ~explicit:true
    (gInject "imset_union_incl")
    [ hole; hole; s1; s2; f; x; hin ]

let imset_incl h = gApp (gInject "imset_incl") [ h ]

let rewrite_set_r seq p =
  gApp ~explicit:true (gInject "rewrite_set_r")
    [ hole; hole; hole; hole; p; seq ]

let rewrite_set_l seq p =
  gApp ~explicit:true (gInject "rewrite_set_l")
    [ hole; hole; hole; hole; p; seq ]
...
let bigcup_cons_setI_subset_pres a f h =
  gApp ~explicit:true
    (gInject "bigcup_cons_setI_subset_pres")
    [ a; hole; f; hole; hole; hole; hole; h ]

let bigcup_cons_setI_subset_compat_backtrack h1 h2 =
  gApp ~explicit:true
    (gInject "bigcup_cons_setI_subset_compat_backtrack")
    [ hole; hole; hole; hole; hole; hole; hole; h1; h2 ]

let bigcup_cons_setI_subset_compat_backtrack_weak h1 h2 =
  gApp ~explicit:true
    (gInject "bigcup_cons_setI_subset_compat_backtrack_weak")
    [ hole; hole; hole; hole; hole; hole; h1; h2 ]

let bigcup_cons_setI_subset_pres_backtrack h =
  gApp ~explicit:true
    (gInject "bigcup_cons_setI_subset_pres_backtrack")
    [ hole; hole; hole; hole; hole; hole; h ]

let bigcup_cons_setI_subset_pres_backtrack_weak h =
  gApp ~explicit:true
    (gInject "bigcup_cons_setI_subset_pres_backtrack_weak")
    [ hole; hole; hole; hole; hole; h ]

let bigcup_nil_setI f l s =
  gApp ~explicit:true (gInject "bigcup_nil_setI") [ hole; hole; f; l; s ]

let isSome_set_eq h1 h2 =
  gApp ~explicit:true (gInject "isSome_set_eq") [ hole; hole; hole; h1; h2 ]

let set_eq_isSome_sound h =
  gApp ~explicit:true (gInject "set_eq_isSome_sound") [ hole; hole; hole; h ]

let set_eq_isSome_complete h =
  gApp ~explicit:true (gInject "set_eq_isSome_complete") [ hole; hole; hole; h ]
