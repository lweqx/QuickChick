...
let semGenSize gen size = gApp (gInject "semGenSize") [ gen; size ]
let semGen gen = gApp (gInject "semGen") [ gen ]
let semReturn x = gApp (gInject "semReturn") [ x ]
let arbitrarySize size = gApp (gInject "arbitrarySize") [ size ]
...
let semBindSize g f size = gApp (gInject "semBindSize") [ g; f; size ]
...
let returnGenSizeMonotonic x = gApp (gInject "returnGenSizeMonotonic") [ x ]
...
let bindMonotonic p s fp =
  gApp ~explicit:true (gInject "bindMonotonic")
    [ hole; hole; hole; hole; p; gFun [ s ] (fun [ x ] -> fp x) ]

let bindMonotonicOpt p s fp =
  gApp ~explicit:true
    (gInject "bindMonotonicOpt")
    [ hole; hole; hole; hole; p; gFun [ s ] (fun [ x ] -> fp x) ]

let bindOptMonotonic p s fp =
  gApp ~explicit:true
    (gInject "bindOptMonotonic")
    [ hole; hole; hole; hole; p; gFun [ s ] (fun [ x ] -> fp x) ]

let bindOptMonotonicOpt p s fp =
  gApp ~explicit:true
    (gInject "bindOptMonotonicOpt")
    [ hole; hole; hole; hole; p; gFun [ s ] (fun [ x ] -> fp x) ]
