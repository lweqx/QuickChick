...
let speclist =
  [
    ( "-s",
      Arg.String (fun name -> sec_name := Some name),
      "Which section's properties to test" );
    ("-v", Arg.Unit (fun _ -> verbose := true), "Verbose mode");
    ( "-failfast",
      Arg.Unit (fun _ -> fail_fast := true),
      "Stop as soon as a problem is detected" );
    ( "-color",
      Arg.Unit (fun _ -> ansi := true),
      "Use colors on an ANSI-compatible terminal" );
    ( "-cmd",
      Arg.String (fun name -> compile_command := name),
      "Compile command for entire directory" );
    ( "-top",
      Arg.String (fun name -> top := name),
      "Name of top-level logical module" );
    ( "-ocamlbuild",
      Arg.String (fun name -> ocamlbuild_args := name),
      "Arguments given to ocamlbuild" );
    ("-nobase", Arg.Unit (fun _ -> nobase := true), "Do not test base mutant");
    ( "-m",
      Arg.Int (fun n -> only_mutant := Some (Num n)),
      "Only test mutant number n" );
    ("-N", Arg.Int (fun n -> maxSuccess := Some n), "Max number of successes");
    ( "-tag",
      Arg.String (fun s -> only_mutant := Some (Tag s)),
      "Only test mutant number with a specific tag" );
    ( "-analysis",
      Arg.Unit (fun _ -> analysis := true),
      "Change output string to JSON format data" );
    ( "-include",
      Arg.String (fun incl -> include_file := Some incl),
      "File containing list of files to be included." );
    ( "-exclude",
      Arg.Rest (fun excl -> excluded := excl :: !excluded),
      "(Deprecated) Files to be excluded. Must be the last argument" );
  ]

let usage_msg = "quickChick options\nMutation testing for current directory";;

Arg.parse speclist
  (fun anon -> Printf.fprintf stderr "Warning: Anonymous argument %s\n" anon)
  usage_msg
...
let ensure_tmpdir_exists () = ignore (ensure_dir_exists tmp_dir)
...
let rec combinations (acc : 'a list) (lists : 'a list list) : 'a list list =
  ...
  | (x :: xs) :: t ->
      combinations (x :: acc) t
      @ add_heads (List.map (fun x' -> x' :: acc) xs) t
...
let rec non_mutants (info : mutant_info) acc muts : mutant_info * string list =
  ...
  | m :: muts' ->
      non_mutants info
        (Printf.sprintf "%s%s%s" m.mut_begin m.mut_body m.mut_end :: acc)
        muts'

let begin_comment () =
  if
    !current_filetype = ".c" || !current_filetype = ".h"
...
let rec all_mutants' (acc : string list) (muts : mutant list) :
    (mutant_info * string list) list =
  match muts with
  | [] -> [ (default_info, List.rev acc) ]
  | { mut_info = info; mut_begin = start; mut_body = code; mut_end = endc }
    :: rest ->
      ...
      @ [
          non_mutants info
            (Printf.sprintf "%s %s %s %s %s" start (end_comment ()) code
               (begin_comment ()) endc
            :: acc)
            rest;
        ]
...
let rec cartesian (lists : 'a list list) : 'a list list =
  ...
  | h :: t ->
      List.concat
        (List.map (fun l -> List.map (fun x -> x :: l) h) (cartesian t))
  | [] -> []
...
let quickCheckFunction () =
  ...
  | None ->
      if !analysis then "quickCheckWith (updAnalysis stdArgs true)"
      else "quickCheck"
  | Some n ->
      if !analysis then
        "quickCheckWith (updAnalysis (updMaxSuccess stdArgs " ^ string_of_int n
        ^ ") true)"
...
let combine_mutants (bms : ('a * (mutant_info * 'a) list) list) :
    (mutant_info * 'a list) list =
  let rec combine_aux bms =
    ...
    | (b, ms) :: bms' ->
        ...
        let mutated_now =
          List.map (fun (info, m) -> (info, m :: non_mutant_rest)) ms
        in
        (* Option 2: Keep base. Mutate later. *)
        let mutated_later =
          List.map (fun (info, ms') -> (info, b :: ms')) (combine_aux bms')
        in
        mutated_now @ mutated_later
  in
  List.rev (combine_aux bms)

let from_cons (l : 'a list) : 'a * 'a list =
  match l with [] -> failwith "Not expecting empty list!" | h :: t -> (h, t)

let from_Some (o : 'a option) : 'a =
  match o with None -> failwith "Not expecting None!" | Some x -> x
...
let mutate_outs handle_section input =
  ...
  let go (sec : section) =
    if handle_section sec.sec_name then (
      ...
      let handle_node (n : node) =
        ...
        | Mutants ms ->
            ...
            let non_mutated, mutants =
              ...
              | (_info, non_mutated) :: mutants ->
                  (non_mutated, List.rev mutants)
      ...
      match
        List.map
          (fun (info, ss) -> (info, String.concat "" ss))
          (combine_mutants (List.map handle_node sec.sec_nodes))
      with
      | (_info, base) :: mutants -> (base, mutants)
      | [] ->
          failwith
            "Internal quickChickTool error: no base mutant after combining")
    else (String.concat "" (List.map output_node sec.sec_nodes), [])
  in
  let result =
    List.map
      (fun (info, ss) -> (info, String.concat "" ss))
      (combine_mutants (List.map go input))
  in
  (result, !things_to_check)

module SS = Set.Make (String)

type 'a file_structure =
  | File of string * 'a
...
let gather_all_vs_from_dir fs =
  ...
  let rec loop fs =
    ...
    | File (s, _) ->
        if
          Filename.check_suffix s ".v"
          && not
               (List.exists
                  (fun x -> Filename.basename x = Filename.basename s)
                  !excluded)
...
let vs_from_file fpath =
  ...
  try
    while true do
      ...
      line |> split_words
      |> List.iter (fun w ->
             if Filename.check_suffix w ".v" then files := w :: !files)
    done;
    assert false
    ...
    | File (s, _) ->
        if
          ...
          && List.exists
               ...
               included
        then all_vs := Filename.chop_suffix s ".v" :: !all_vs
    | Dir (_, fsss) -> List.iter loop fsss
  in
  loop fs;
  !all_vs
...
let confirm_results e =
  let failed s =
    ...
    if !fail_fast then exit 1 else something_failed := true
  in
  if test_results.inconclusive > 0 then failed "Inconclusive test"
  else
    match e with
    | ExpectOnlySuccesses ->
        if test_results.failed > 0 then failed "Test failed in base"
    | ExpectSomeFailure ->
        if test_results.failed = 0 then failed "No tests failed for this mutant"
...
let run_and_show_output_on_failure command msg =
  ...
  with End_of_file ->
    ...
    let result =
      ...
      | Unix.WEXITED i ->
          List.rev (Printf.sprintf "Exited with status %d" i :: !res)
      | Unix.WSIGNALED i -> List.rev (Printf.sprintf "Killed (%d)" i :: !res)
      | Unix.WSTOPPED i -> List.rev (Printf.sprintf "Stopped (%d)" i :: !res)
    in
    if stat <> Unix.WEXITED 0 || !verbose then
      List.iter
        (fun s ->
          ...
          print_newline ())
        result;
    if stat = Unix.WEXITED 0 then () else failwith msg
...
let system args =
  ...
  | e ->
      highlight Failure
        (Printf.sprintf "Command failed: %s" (string_of_process_status e));
      exit 1
...
let compile_and_run where e : unit =
  ...
  let ocamlbuild_cmd =
    Printf.sprintf
      "ocamlbuild -use-ocamlfind -pkg zarith -cflag -rectypes %s %s.native"
      !ocamlbuild_args
      (Filename.chop_suffix temporary_file ".v")
  in
  run_and_show_output_on_failure ocamlbuild_cmd "Ocamlbuild failure";

  reset_test_results ();

  let run_command =
    Printf.sprintf "./%s.native" (Filename.chop_suffix temporary_file ".v")
  in
...
let rec parse_file_or_dir file_name =
  try
    ...
    if is_dir file_name then (
      if
        is_prefix tmp_dir (Filename.basename file_name)
      ...
      then None
        ...
        let parsed =
          List.map
            (fun s -> parse_file_or_dir (file_name ^ "/" ^ s))
            (Array.to_list ls)
        in
    ...
    then
      ...
      let handle =
        (Filename.check_suffix file_name ".v"
      ...
      if handle then (
        ...
        let result =
          try program lexer lexbuf
          with exn ->
            Printf.printf "Parse error when reading file: %s\n" file_name;
            raise exn
        in
        let collapse l =
          let rec loop acc acc_text l =
            ...
            | n :: l' ->
                let text_acc_node =
                  Text (String.concat "" (List.rev acc_text))
                in
        ...
        let result =
          List.map
            (fun sec -> { sec with sec_nodes = collapse sec.sec_nodes })
            result
        in

        let fix_extends extends = split_words (String.concat "" extends) in
        let result =
          List.map
            (fun sec ->
              {
                ...
                sec_extends =
                  option_map
                    (fun e ->
                      { e with ext_extends = fix_extends e.ext_extends })
                    sec.sec_extends;
              })
            result
        in
          ...
          | sec :: ss ->
              {
                ...
                sec_nodes = sec.sec_nodes;
              }
              :: ss
          | _ -> failwith "Empty section list?"
        in
...
let rec section_length_of_fs fs =
  ...
  | Dir (_, fss) -> List.fold_left ( + ) 0 (List.map section_length_of_fs fss)

(* TODO: "trim" is a confusing name for this function! *)
let trim s = match split_words s with [] -> "" | h :: _ -> h
...
let sec_find sec_graph s =
  ...
  with Not_found ->
    ...
    failwith
      (Printf.sprintf
         "QuickChick: Didn't find section called %s (available sections: %s)\n"
         s keys)
...
let build_sec_graph fs =
  ...
  let rec populate_hashtbl_sections (sections : section list) =
    ...
    | sec :: rest ->
        let extends =
          match sec.sec_extends with Some ext -> ext.ext_extends | None -> []
        in
        Hashtbl.add sec_graph (trim sec.sec_name) extends;
        populate_hashtbl_sections rest
  in
...
let handle_section sec_graph sn' =
  if !verbose then (
    Printf.printf "Asking for section %s\n" sn';
    flush_all ());
  let sn' = trim sn' in
  match !sec_name with
  ...
  let rec loop (fs : section list file_structure) =
    ...
    | File (s, ss) -> (
        ...
        | (_info, base) :: muts, things_to_check ->
            ...
            all_things_to_check :=
              List.map (fun x -> (s, x)) things_to_check @ !all_things_to_check;
            (* debug "Number of tests: %d\n%s\n" (List.length things_to_check) (String.concat "\n" things_to_check); *)
            ( File (s, base),
              List.map (fun (info, m) -> (info, File (s, m))) muts )
    ...
    | Dir (s, fss) -> (
        ...
        let rec all_mutant_fs (bmfs : ('a * (mutant_info * 'a) list) list) =
          ...
          | (b, mfs) :: bmfs' ->
              ...
              let mutated_now =
                List.map (fun (info, mf) -> (info, mf :: non_mutant_rest)) mfs
              in
              let mutated_later =
                List.map
                  (fun (info, mfs') -> (info, b :: mfs'))
                  (all_mutant_fs bmfs')
              in
              mutated_now @ mutated_later
        in
        match List.rev (all_mutant_fs bmfs) with
        | (_, base) :: muts ->
...
let string_of_tag ms = match ms with None -> "<no tag>" | Some t -> t
...
let starts_with ~prefix b =
  ...
  | false ->
      let rec f j =
        if j >= len then true else prefix.[j] = b.[j] && f (j + 1)
      in
      f 0
...
let main =
  ...
  if !excluded <> [] then
    output_string stderr "Warning: -exclude option is deprecated\n";
  ...
  match fs with
  | File (_, _) -> failwith ". can never be a file. Right?"
  | Dir (s, fss) ->
      (let (base, dir_mutants), all_things_to_check =
         calc_dir_mutants sec_graph fs
       in
       ...
       let rec output_mut_dir tmp_dir fs =
         ...
         | File (s, out_data) ->
             ...
             then ignore (write_file out_file out_data)
         | Dir (s, fss) ->
       ...
       let imports =
         List.map
           (fun s -> (if !top = "" then "" else !top ^ ".") ^ mk_import s)
           all_vs
       in
       ...
       let test_names, tests =
         ...
         let make_test i (f, s) : string * string =
           ...
           let testname =
             ...
             trim (Filename.basename (Filename.chop_suffix f ".v")) ^ "." ^ s
           in
           let structured_structured_output =
             Printf.sprintf
               "Definition test%d := print_extracted_coq_string \
                (\"{\"\"name\"\": \"\"%s\"\", \"  ++ show (withTime(fun tt => \
                %s %s)) ++ \"}"
           ...
           let readable_output =
             Printf.sprintf
               "Definition test%d := print_extracted_coq_string (\"Checking \
                %s...\" ++ newline ++ show (%s %s))%%string.\n"
               i testname (quickCheckFunction ()) testname
           in
           ( Printf.sprintf "test%d" i,
             if !analysis then structured_structured_output else readable_output
           )
         in
         List.split (List.mapi make_test all_things_to_check)
       in
       ...
       let tmp_file_data =
         ...
         ^ String.concat "\n" tests ^ "\n"
         ^
         if extractions @ test_names <> [] then
           "Separate Extraction "
           ^ String.concat " " extractions
           ^ " "
       ...
       (* Base mutant *)
       if (not !nobase) && !only_mutant = None then (
       ...
       List.iteri
         (fun i (info, m) ->
           ...
           then (
             Printf.printf "\n";
             let t =
               match info.tag with None -> "" | Some s -> ": " ^ String.trim s
             in
             (* TODO: The line number info should also include the
                file name! *)
             highlight Header
               (Printf.sprintf "Testing mutant %d (%s: line %d)%s" i
                  info.file_name info.line_number t);
             ensure_tmpdir_exists ();
             (* Entire file structure is copied *)
